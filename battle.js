...
function checkShot(event){
    ...
    if(matrixAttack[x][y] === "ship"){
        ...
        new Noty({
            customClass:'noty_body',
            theme:"mint" ,
            text:"Tu dispro cayo en el agua",
        }).show();
...
function shotPc(){
    ...
    if(matrix[x][y] === "ship"){
        ...
        new Noty({
            text:'El dispro del Pc cayo en el agua',
        }).show();
        matrix[x][y] = "miss";
...
function imgBoat() {
    let barcos = document.getElementsByClassName('img');
    for (let i = 0; i < barcos.length; i++) {
      let barco = barcos[i];
        barco.addEventListener('click' , (function(name){
            return function(){
                 new Noty({
                    text:`selecionaste ${name}`,
                    layout:'top',            
                }).show();
            }
        })(barco.name))
